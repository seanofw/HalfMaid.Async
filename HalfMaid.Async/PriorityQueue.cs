#if NETFRAMEWORK
#error .NET Framework is not supported.
#endif
#if !NET6_0_OR_GREATER

// This is a hacked copy of PriorityQueue.cs from the .NET 6+ source code.
// It provides backward-compatibility for .NET Core 2.0 through .NET 5.0.
// Only include this if you're using .NET older than .NET 6.
//
...
namespace System.Collections.Generic
{
    ...
    internal class PriorityQueue<TElement, TPriority>
    {
        ...
        public void EnqueueRange(IEnumerable<(TElement Element, TPriority Priority)> items)
        {
            ...
            if (!ReferenceEquals(collection, null) && (count = collection.Count) > _nodes.Length - _size)
            {
        ...
        private void MoveUpCustomComparer((TElement Element, TPriority Priority) node, int nodeIndex)
        {
            ...
            Debug.Assert(!ReferenceEquals(_comparer, null));
            Debug.Assert(0 <= nodeIndex && nodeIndex < _size);
